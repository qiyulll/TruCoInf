...
class GenerateRequest(BaseModel):
    ...
    min_new_tokens: Optional[int] = Field(default=1)  # Avoid stopping on EOS before any visible token is generated
...
async def api_generate(request: GenerateRequest):
    ...
    if max_length is not None:
        length_param = {"max_length": max_length}
    else:
        safe_min_new_tokens = max(0, min(int(min_new_tokens or 0), int(max_new_tokens or 0)))
        length_param = {
