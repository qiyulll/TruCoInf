...
def update_input_ids_and_model_kwargs(model, state):
    ...
    eos_token_id_tensor = state.get("eos_token_id_tensor")

    # If pad_token_id is missing, fallback to eos_token_id if available.
    if pad_token_id is None:
        if eos_token_id_tensor is not None:
            pad_token_id = int(eos_token_id_tensor.item()) if hasattr(eos_token_id_tensor, "item") else int(eos_token_id_tensor)
            state["pad_token_id"] = pad_token_id
        elif hasattr(model, "generation_config") and getattr(model.generation_config, "pad_token_id", None) is not None:
