...
{
    // Ensure previous GPU operations complete before copy
...
std::vector<int> FrTensor::to_int_host() const
{
    ...
    scalar_to_int_kernel<<<(size+FrNumThread-1)/FrNumThread,FrNumThread>>>(gpu_data, int_gpu_data, size);
    cudaDeviceSynchronize();
    cudaMemcpy(out.data(), int_gpu_data, sizeof(int) * size, cudaMemcpyDeviceToHost);
    cudaFree(int_gpu_data);
    return out;
}
