...
std::vector<Fr_t> Polynomial::to_host_coeffs() const
{
    if (degree_ < 0 || coefficients_ == nullptr) return {};
    std::vector<Fr_t> out(static_cast<size_t>(degree_ + 1));
    cudaMemcpy(out.data(), coefficients_, sizeof(Fr_t) * static_cast<size_t>(degree_ + 1), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    return out;
}
