...
{
    vector<Fr_t> u_cat = concatenate(vector<vector<Fr_t>>({c.u[0], c.u[1]}));
...
Fr_t multi_hadamard_sumchecks_prove_fs(const Fr_t& claim, const vector<FrTensor>& Xs, const vector<Fr_t>& u, Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix)
{
    auto N = Xs[0].size;
    if (N == 1) return claim;
    if ((N & (N - 1)) != 0) throw std::runtime_error("multi_hadamard_sumchecks_prove_fs: N not power of 2");
    uint N_out = N >> 1;
    ...
    for (uint i = 1; i < Xs.size(); ++i) {
        if (Xs[i].size != N) throw std::runtime_error("multi_hadamard_sumchecks_prove_fs: size mismatch");
        FrTensor A0(N_out), A1(N_out);
        hadamard_split_kernel<<<(N_out+FrNumThread-1)/FrNumThread,FrNumThread>>>(Xs[i].gpu_data, A0.gpu_data, A1.gpu_data, N_out);
        cudaDeviceSynchronize();
        out.push_back(out.back() * A1);
        for (int j = static_cast<int>(i); j >= 1; --j) {
            out[j] *= A0;
            out[j] += out[j - 1] * A1;
        }
        out[0] *= A0;
    ...
    auto bytes = ps_serialize_poly(poly);
    ps_write_bytes(proof_messages, bytes.data(), bytes.size());
    transcript.absorb(bytes);

    auto p = poly * Polynomial::eq(u.back());
    if (claim != p({0, 0, 0, 0, 0, 0, 0, 0}) + p({1, 0, 0, 0, 0, 0, 0, 0})) throw std::runtime_error("multi_hadamard_sumchecks_prove_fs: claim check failed");

    auto v = transcript.challenge_fr(label_prefix + "/hadamard/v");
    auto new_claim = poly(v);

    vector<FrTensor> new_Xs;
    for (auto& X : Xs) {
        FrTensor new_X(N_out);
        hadamard_reduce_kernel<<<(N_out+FrNumThread-1)/FrNumThread,FrNumThread>>>(X.gpu_data, v, new_X.gpu_data, N_out);
        cudaDeviceSynchronize();
        new_Xs.push_back(new_X);
    }
    return multi_hadamard_sumchecks_prove_fs(new_claim, new_Xs, u_, transcript, proof_messages, label_prefix);
}
...
Fr_t multi_hadamard_sumchecks_verify_fs(const Fr_t& claim, const vector<FrTensor>& Xs, const vector<Fr_t>& u, Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix)
{
    auto N = Xs[0].size;
    if (N == 1) return claim;
    if ((N & (N - 1)) != 0) throw std::runtime_error("multi_hadamard_sumchecks_verify_fs: N not power of 2");
    uint N_out = N >> 1;

    size_t off0 = proof_off;
    Polynomial poly = ps_deserialize_poly(proof_messages, proof_off);
    transcript.absorb(proof_messages.data() + off0, proof_off - off0);

    auto p = poly * Polynomial::eq(u.back());
    if (claim != p({0, 0, 0, 0, 0, 0, 0, 0}) + p({1, 0, 0, 0, 0, 0, 0, 0})) throw std::runtime_error("multi_hadamard_sumchecks_verify_fs: claim check failed");
    ...
    vector<Fr_t> u_(u.begin(), u.end() - 1);
    return multi_hadamard_sumchecks_verify_fs(new_claim, new_Xs, u_, transcript, proof_messages, proof_off, label_prefix);
}
