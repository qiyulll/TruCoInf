...
static uint next_lookup_proof_size(uint value, uint table_size)
{
    uint target = 1 << ceilLog2(value);
    if (target < table_size) target = table_size;
    while (target % table_size != 0) target <<= 1;
    return target;
}

KERNEL void pad_fr_tensor_kernel(const Fr_t* in_ptr, Fr_t* out_ptr, uint in_size, uint out_size)
{
    uint tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < out_size) {
        out_ptr[tid] = tid < in_size ? in_ptr[tid] : blstrs__scalar__Scalar_ZERO;
    }
}

static FrTensor pad_to_size(const FrTensor& input, uint target_size)
{
    if (input.size == target_size) return FrTensor(input);
    if (input.size > target_size) throw std::runtime_error("pad_to_size: input larger than target");

    FrTensor out(target_size);
    uint block_size = 256;
    pad_fr_tensor_kernel<<<(target_size + block_size - 1) / block_size, block_size>>>(
        input.gpu_data, out.gpu_data, input.size, target_size
    );
...
vector<Claim> Rescaling::prove(const FrTensor& X, const FrTensor& X_)
{
    ...
    uint proof_size = next_lookup_proof_size(X.size, tl_rem.table.size);
    auto u = random_vec(ceilLog2(proof_size));
    auto v = random_vec(ceilLog2(proof_size));
    ...
    vector<Polynomial>& proof)
{
    ...
    uint proof_size = next_lookup_proof_size(X.size, tl_rem.table.size);
    if (u.size() != ceilLog2(proof_size)) throw std::runtime_error("Rescaling::prove: u size mismatch");
    if (v.size() != ceilLog2(proof_size)) throw std::runtime_error("Rescaling::prove: v size mismatch");

    auto X_padded = pad_to_size(X, proof_size);
    auto X_padded_ = pad_to_size(X_, proof_size);
    auto rem = pad_to_size(*rem_tensor_ptr, proof_size);
    auto m = tl_rem.prep(rem);

    if (X_padded(u) != X_padded_(u) * Fr_t({scaling_factor, 0, 0, 0, 0, 0, 0, 0}) + rem(u)) {
