        ...
        const vector<Fr_t>& u, const vector<Fr_t>& v, vector<Polynomial>& proof)
{
    ...
    // We need D to be a power of 2, and D >= N with D % N == 0.
    // For LLaMA and similar large tables (N is large) where D(=seq_len*hidden_dim) may be < N,
    // we pad D to max(nextPow2(D), N) to satisfy constraints.
    const uint D_pow2 = 1u << ceilLog2(D);
    uint D_target = D_pow2;
