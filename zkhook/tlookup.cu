...
// ===== Transcript-based Fiat–Shamir (over prover messages) variants =====

static const Fr_t PS_ZERO {0, 0, 0, 0, 0, 0, 0, 0};
static const Fr_t PS_ONE  {1, 0, 0, 0, 0, 0, 0, 0};

static Fr_t tLookup_phase2_fs_prove(
    const Fr_t& claim,
    const FrTensor& A, const FrTensor& S,
    const FrTensor& B, const FrTensor& T, const FrTensor& m,
    const Fr_t& alpha_, const Fr_t& beta,
    ...
    uint depth2)
{
    if (depth2 == 0) return claim;
    auto p = tLookup_phase2_step_poly(A, S, B, T, m, alpha_, beta, inv_size_ratio, alpha_sq, u);
    auto bytes = ps_serialize_poly(p);
    ps_write_bytes(proof_messages, bytes.data(), bytes.size());
    transcript.absorb(bytes);
    if (claim != p(PS_ZERO) + p(PS_ONE)) throw std::runtime_error("tLookup_phase2_fs_prove: claim != p(0)+p(1)");
    auto v = transcript.challenge_fr(label_prefix + "/tlookup/v2");
    ...
    cudaDeviceSynchronize();
    return tLookup_phase2_fs_prove(
        p(v),
        new_A, new_S, new_B, new_T, new_m,
        alpha_ * Polynomial::eq(u.back(), v),
        beta,
        inv_size_ratio,
        alpha_sq * Polynomial::eq(u.back(), v),
        {u.begin(), u.end() - 1},
        transcript, proof_messages, label_prefix,
        depth2 - 1
    );
}

static Fr_t tLookup_phase2_fs_verify(
    const Fr_t& claim,
    const FrTensor& A, const FrTensor& S,
    const FrTensor& B, const FrTensor& T, const FrTensor& m,
    const Fr_t& alpha_, const Fr_t& beta,
    const Fr_t& inv_size_ratio, const Fr_t& alpha_sq,
    const vector<Fr_t>& u,
    Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off,
    const std::string& label_prefix,
    uint depth2)
{
    if (depth2 == 0) return claim;
    size_t off0 = proof_off;
    auto p = ps_deserialize_poly(proof_messages, proof_off);
    transcript.absorb(proof_messages.data() + off0, proof_off - off0);
    if (claim != p(PS_ZERO) + p(PS_ONE)) throw std::runtime_error("tLookup_phase2_fs_verify: claim != p(0)+p(1)");
    auto v = transcript.challenge_fr(label_prefix + "/tlookup/v2");
    ...
    cudaDeviceSynchronize();
    return tLookup_phase2_fs_verify(
        p(v),
        new_A, new_S, new_B, new_T, new_m,
        alpha_ * Polynomial::eq(u.back(), v),
        beta,
        ...
        alpha_sq * Polynomial::eq(u.back(), v),
        {u.begin(), u.end() - 1},
        transcript, proof_messages, proof_off, label_prefix,
        depth2 - 1
    );
}

static Fr_t tLookup_phase1_fs_prove(
    const Fr_t& claim,
    const FrTensor& A, const FrTensor& S,
    const FrTensor& B, const FrTensor& T, const FrTensor& m,
    const Fr_t& alpha, const Fr_t& beta,
    const Fr_t& C, const Fr_t& inv_size_ratio, const Fr_t& alpha_sq,
    const vector<Fr_t>& u,
    Transcript& transcript, std::vector<uint8_t>& proof_messages,
    const std::string& label_prefix,
    uint depth1, uint depth2)
{
    if (depth1 == 0) {
        return tLookup_phase2_fs_prove(claim, A, S, B, T, m, alpha, beta, inv_size_ratio, alpha_sq, u, transcript, proof_messages, label_prefix, depth2);
    }
    auto p = tLookup_phase1_step_poly(A, S, alpha, beta, C, u);
    auto bytes = ps_serialize_poly(p);
    ps_write_bytes(proof_messages, bytes.data(), bytes.size());
    transcript.absorb(bytes);
    if (claim != p(PS_ZERO) + p(PS_ONE)) throw std::runtime_error("tLookup_phase1_fs_prove: claim != p(0)+p(1)");
    auto v = transcript.challenge_fr(label_prefix + "/tlookup/v1");

    FrTensor new_A(A.size >> 1), new_S(S.size >> 1);
    tLookup_phase1_reduce_kernel<<<(A.size+FrNumThread-1)/FrNumThread,FrNumThread>>>(
        A.gpu_data, S.gpu_data, new_A.gpu_data, new_S.gpu_data, v, A.size >> 1
    );
    cudaDeviceSynchronize();
    return tLookup_phase1_fs_prove(
        p(v),
        new_A, new_S, B, T, m,
        alpha * Polynomial::eq(u.back(), v),
        beta,
        C * TWO_INV,
        inv_size_ratio,
        alpha_sq,
...
static Fr_t tLookup_phase1_fs_verify(
    const Fr_t& claim,
    const FrTensor& A, const FrTensor& S,
    const FrTensor& B, const FrTensor& T, const FrTensor& m,
    const Fr_t& alpha, const Fr_t& beta,
...
{
    if (depth1 == 0) {
        return tLookup_phase2_fs_verify(claim, A, S, B, T, m, alpha, beta, inv_size_ratio, alpha_sq, u, transcript, proof_messages, proof_off, label_prefix, depth2);
    }
    size_t off0 = proof_off;
    auto p = ps_deserialize_poly(proof_messages, proof_off);
    transcript.absorb(proof_messages.data() + off0, proof_off - off0);
    if (claim != p(PS_ZERO) + p(PS_ONE)) throw std::runtime_error("tLookup_phase1_fs_verify: claim != p(0)+p(1)");
    auto v = transcript.challenge_fr(label_prefix + "/tlookup/v1");
    ...
    cudaDeviceSynchronize();
    return tLookup_phase1_fs_verify(
        p(v),
        new_A, new_S, B, T, m,
        alpha * Polynomial::eq(u.back(), v),
        beta,
...
Fr_t tLookup::prove_fs(const FrTensor& S, const FrTensor& m, const Fr_t& alpha, const Fr_t& beta,
    const vector<Fr_t>& u, Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix) const
{
    const uint D = S.size;
    const uint N = m.size;
    if (m.size != table.size) throw std::runtime_error("tLookup::prove_fs: m.size != table.size");
    if (D != 1 << ceilLog2(D) || N != 1 << ceilLog2(N) || D % N != 0) throw std::runtime_error("tLookup::prove_fs: bad sizes");
    if (u.size() != ceilLog2(D)) throw std::runtime_error("tLookup::prove_fs: u.size mismatch");

    FrTensor A(D), B(N);
    tlookup_inv_kernel<<<(D+FrNumThread-1)/FrNumThread,FrNumThread>>>(S.gpu_data, beta, A.gpu_data, D);
    cudaDeviceSynchronize();
    tlookup_inv_kernel<<<(N+FrNumThread-1)/FrNumThread,FrNumThread>>>(table.gpu_data, beta, B.gpu_data, N);
    cudaDeviceSynchronize();
    ...
    return tLookup_phase1_fs_prove(claim, A, S, B, table, m,
        alpha, beta, C, N_Fr / D_Fr, alpha_sq,
        u, transcript, proof_messages, label_prefix, depth1, depth2);
}

Fr_t tLookup::verify_fs(const FrTensor& S, const FrTensor& m, const Fr_t& alpha, const Fr_t& beta,
    const vector<Fr_t>& u, Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix) const
{
    const uint D = S.size;
    const uint N = m.size;
    if (m.size != table.size) throw std::runtime_error("tLookup::verify_fs: m.size != table.size");
    if (D != 1 << ceilLog2(D) || N != 1 << ceilLog2(N) || D % N != 0) throw std::runtime_error("tLookup::verify_fs: bad sizes");
    if (u.size() != ceilLog2(D)) throw std::runtime_error("tLookup::verify_fs: u.size mismatch");
    ...
    return tLookup_phase1_fs_verify(claim, A, S, B, table, m,
        alpha, beta, C, N_Fr / D_Fr, alpha_sq,
        u, transcript, proof_messages, proof_off, label_prefix, depth1, depth2);
}
...
KERNEL void tlookup_pad_to_kernel(const Fr_t* in_data, Fr_t* out_data, uint in_size, uint out_size, Fr_t pad_val)
{
    const uint tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid >= out_size) return;
    out_data[tid] = (tid < in_size) ? in_data[tid] : pad_val;
}

static FrTensor tlookup_pad_1d_to(const FrTensor& in, uint out_size, const Fr_t& pad_val)
{
    if (out_size < in.size) throw std::runtime_error("tlookup_pad_1d_to: out_size < in.size");
    FrTensor out(out_size);
    tlookup_pad_to_kernel<<<(out_size + FrNumThread - 1) / FrNumThread, FrNumThread>>>(
        in.gpu_data, out.gpu_data, in.size, out_size, pad_val
    );
    cudaDeviceSynchronize();
    return out;
}

static uint tlookup_mapping_target_size(uint D, uint N)
{
    uint D_target = 1u << ceilLog2(D);
    if (D_target < N) D_target = N;
    while (D_target % N != 0) D_target <<= 1;
    return D_target;
}
        ...
        const vector<Fr_t>& u, const vector<Fr_t>& v, vector<Polynomial>& proof)
{
    ...
    const uint D_target = tlookup_mapping_target_size(D, N);
    auto S_in_ = tlookup_pad_1d_to(S_in, D_target, table(0));
    auto m_ = prep(S_in_);

    vector<Fr_t> u_lookup = (u.size() == ceilLog2(D_target)) ? u : random_vec(ceilLog2(D_target));
    vector<Fr_t> v_lookup = (v.size() == ceilLog2(D_target)) ? v : random_vec(ceilLog2(D_target));
    return tLookup::prove(S_in_, m_, alpha, beta, u_lookup, v_lookup, proof);
}

Fr_t tLookupRangeMapping::prove_fs(const FrTensor& S_in, const FrTensor& S_out, const FrTensor& m,
        const Fr_t& r, const Fr_t& alpha, const Fr_t& beta,
        const vector<Fr_t>& u, Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix) const
{
    const uint D = S_in.size;
    if (m.size != table.size) throw std::runtime_error("tLookupRangeMapping::prove_fs: m.size != table.size");
    const uint N = m.size;

    const uint D_target = tlookup_mapping_target_size(D, N);
    if (D_target != D) {
        auto S_in_  = tlookup_pad_1d_to(S_in,  D_target, table(0));
        auto S_out_ = tlookup_pad_1d_to(S_out, D_target, mapped_vals(0));
        FrTensor m_(m);
        tlookuprange_pad_m<<<1,1>>>(m_.gpu_data, 0, D_target - D);
        cudaDeviceSynchronize();
        return prove_fs(S_in_, S_out_, m_, r, alpha, beta, u, transcript, proof_messages, label_prefix);
    }

    if (N != 1 << ceilLog2(N) || D % N != 0) throw std::runtime_error("tLookupRangeMapping::prove_fs: bad sizes");
    if (u.size() != ceilLog2(D)) throw std::runtime_error("tLookupRangeMapping::prove_fs: u.size mismatch");

    FrTensor A(D), B(N);
    auto S_com = S_in + S_out * r;
    auto T_com = table + mapped_vals * r;
    tlookup_inv_kernel<<<(D+FrNumThread-1)/FrNumThread,FrNumThread>>>(S_com.gpu_data, beta, A.gpu_data, D);
    cudaDeviceSynchronize();
    tlookup_inv_kernel<<<(N+FrNumThread-1)/FrNumThread,FrNumThread>>>(T_com.gpu_data, beta, B.gpu_data, N);
    cudaDeviceSynchronize();
    ...
    return tLookup_phase1_fs_prove(claim, A, S_com, B, T_com, m,
        alpha, beta, C, N_Fr / D_Fr, alpha_sq,
        u, transcript, proof_messages, label_prefix, depth1, depth2);
}

Fr_t tLookupRangeMapping::verify_fs(const FrTensor& S_in, const FrTensor& S_out, const FrTensor& m,
        const Fr_t& r, const Fr_t& alpha, const Fr_t& beta,
        const vector<Fr_t>& u, Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix) const
{
    const uint D = S_in.size;
    if (m.size != table.size) throw std::runtime_error("tLookupRangeMapping::verify_fs: m.size != table.size");
    const uint N = m.size;
    ...
    if (D_target != D) {
        ...
        tlookuprange_pad_m<<<1,1>>>(m_.gpu_data, 0, D_target - D);
        cudaDeviceSynchronize();
        return verify_fs(S_in_, S_out_, m_, r, alpha, beta, u, transcript, proof_messages, proof_off, label_prefix);
    }

    if (N != 1 << ceilLog2(N) || D % N != 0) throw std::runtime_error("tLookupRangeMapping::verify_fs: bad sizes");
    if (u.size() != ceilLog2(D)) throw std::runtime_error("tLookupRangeMapping::verify_fs: u.size mismatch");
    ...
    return tLookup_phase1_fs_verify(claim, A, S_com, B, T_com, m,
        alpha, beta, C, N_Fr / D_Fr, alpha_sq,
        u, transcript, proof_messages, proof_off, label_prefix, depth1, depth2);
}
