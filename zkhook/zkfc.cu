...
vector<Claim> zkFC::prove(const FrTensor& X, const FrTensor& Y) const
{
    ...
    return prove(X, Y, u_batch, u_input, u_output, proof);

}

vector<Claim> zkFC::prove(
    const FrTensor& X,
    const FrTensor& Y,
    ...
    vector<Polynomial>& proof) const
{
    if (has_bias) throw std::runtime_error("Cleaned-up version not implemented for zkFC with bias. Use zkFCStacked instead.");
    uint batchSize = X.size / inputSize;
    if (u_batch.size() != ceilLog2(batchSize)) throw std::runtime_error("zkFC::prove: u_batch size mismatch");
    if (u_input.size() != ceilLog2(inputSize)) throw std::runtime_error("zkFC::prove: u_input size mismatch");
    if (u_output.size() != ceilLog2(outputSize)) throw std::runtime_error("zkFC::prove: u_output size mismatch");
    ...
    if (claim_X * claim_W != final_claim) {
        throw std::runtime_error("zkFC::prove: claim_X * claim_W != final_claim");
    }
...
Fr_t zkip_prove_fs(const Fr_t& claim, const FrTensor& a, const FrTensor& b, Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix, std::vector<Fr_t>* out_challenges)
{
    // Iterative version until tensor size reduces to 1.
    // Use pointers to avoid FrTensor assignment issues with different sizes
    Fr_t cur_claim = claim;
    std::unique_ptr<FrTensor> cur_a = std::make_unique<FrTensor>(a);
    std::unique_ptr<FrTensor> cur_b = std::make_unique<FrTensor>(b);
    
    std::vector<Fr_t> challenges;
    
    // Special case: no sumcheck needed when size=1
    if (cur_a->size == 1) {
        if (out_challenges) {
            *out_challenges = {};
        }
    ...
    int round = 0;
    while (cur_a->size > 1) {
        auto p = zkip_step_poly(*cur_a, *cur_b, TEMP_ZERO);
        auto bytes = ps_serialize_poly(p);
        ps_write_bytes(proof_messages, bytes.data(), bytes.size());
        transcript.absorb(bytes);

        auto r = transcript.challenge_fr(label_prefix + "/zkip/r");
        challenges.push_back(r);
        
        Fr_t p0_p1 = p(TEMP_ZERO) + p(TEMP_ONE);
        if (cur_claim != p0_p1) {
            std::cerr << "[DEBUG] Mismatch at round " << round << ":" << std::endl;
            std::cerr << "  cur_claim = " << cur_claim << std::endl;
            std::cerr << "  p(0)+p(1) = " << p0_p1 << std::endl;
            throw std::runtime_error("zkip_prove_fs: claim != p(0)+p(1)");
        }
        round++;

        uint N_in = cur_a->size;
        uint N_out = (1 << ceilLog2(cur_a->size)) >> 1;
        auto new_a = std::make_unique<FrTensor>(N_out);
        auto new_b = std::make_unique<FrTensor>(N_out);
        zkip_reduce_kernel<<<(N_out+FrNumThread-1)/FrNumThread,FrNumThread>>>(cur_a->gpu_data, cur_b->gpu_data, new_a->gpu_data, new_b->gpu_data, r, N_in, N_out);
        cudaDeviceSynchronize();
        cur_a = std::move(new_a);
        cur_b = std::move(new_b);
        cur_claim = p(r);
    }
    
    // Return challenges in reverse order (to match the convention used elsewhere)
    if (out_challenges) {
        *out_challenges = std::vector<Fr_t>(challenges.rbegin(), challenges.rend());
    }
...
Fr_t zkip_verify_fs(const Fr_t& claim, const FrTensor& a, const FrTensor& b, Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix)
{
    ...
    while (cur_a->size > 1) {
        ...
        auto r = transcript.challenge_fr(label_prefix + "/zkip/r");
        if (cur_claim != p(TEMP_ZERO) + p(TEMP_ONE)) throw std::runtime_error("zkip_verify_fs: claim != p(0)+p(1)");
...
vector<Claim> zkFC::prove_fs(const FrTensor& X, const FrTensor& Y, Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix) const
{
    if (has_bias) throw std::runtime_error("zkFC::prove_fs: bias not supported");
    uint batchSize = X.size / inputSize;

    // Generate u_batch and u_output challenges first
    auto u_batch = transcript.challenge_vec(label_prefix + "/zkfc/u_batch", ceilLog2(batchSize));
    auto u_output = transcript.challenge_vec(label_prefix + "/zkfc/u_output", ceilLog2(outputSize));

    auto claim = Y.multi_dim_me({u_batch, u_output}, {batchSize, outputSize});
    auto X_reduced = X.partial_me(u_batch, batchSize, inputSize);
    auto W_reduced = weights.partial_me(u_output, outputSize, 1);

    // Run zkip and collect the u_input challenges generated during sumcheck
    std::vector<Fr_t> u_input;
    auto final_claim = zkip_prove_fs(claim, X_reduced, W_reduced, transcript, proof_messages, label_prefix, &u_input);
    
    // Now use the u_input challenges that were actually generated during sumcheck
    auto claim_X = X.multi_dim_me({u_batch, u_input}, {batchSize, inputSize});
    auto claim_W = weights.multi_dim_me({u_input, u_output}, {inputSize, outputSize});
    
    if (claim_X * claim_W != final_claim) {
        throw std::runtime_error("zkFC::prove_fs: claim mismatch");
    }
...
vector<Claim> zkFC::verify_fs(const FrTensor& X, const FrTensor& Y, Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix) const
{
    if (has_bias) throw std::runtime_error("zkFC::verify_fs: bias not supported");
    uint batchSize = X.size / inputSize;

    auto u_batch = transcript.challenge_vec(label_prefix + "/zkfc/u_batch", ceilLog2(batchSize));
    auto u_input = transcript.challenge_vec(label_prefix + "/zkfc/u_input", ceilLog2(inputSize));
    auto u_output = transcript.challenge_vec(label_prefix + "/zkfc/u_output", ceilLog2(outputSize));
    ...
    auto final_claim = zkip_verify_fs(claim, X_reduced, W_reduced, transcript, proof_messages, proof_off, label_prefix);
    auto claim_X = X.multi_dim_me({u_batch, u_input}, {batchSize, inputSize});
    auto claim_W = weights.multi_dim_me({u_input, u_output}, {inputSize, outputSize});
    if (claim_X * claim_W != final_claim) throw std::runtime_error("zkFC::verify_fs: claim mismatch");
