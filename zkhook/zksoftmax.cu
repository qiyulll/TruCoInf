...
least_significant_segments(), other_segments()
{
    ...
    {
        // Use dynamic allocation instead of VLA (VLA is non-standard in C++)
        std::vector<int> mvals(bs[i], 0);
        mvals[0] = 1;  // Only first element is 1, rest are 0
        FrTensor mapped_values(bs[i], mvals.data());
        other_segments.push_back({0, bs[i], mapped_values});
        // cout << "======== Debug info for segment " << i << endl;
        // // copy mapped_values.gpu_data to cpu
        // Fr_t* mapped_values_cpu = new Fr_t[bs[i]];
...
Fr_t zkSoftmax::prove_fs(
    const FrTensor& Y, const FrTensor& X, const FrTensor& shift, const FrTensor& X_shifted,
    const vector<FrTensor>& X_segments, const vector<FrTensor>& Y_segments, const vector<FrTensor>& m_segments,
    Transcript& transcript, std::vector<uint8_t>& proof_messages, const std::string& label_prefix)
{
    uint N = Y.size;
    if (X.size != N) throw std::invalid_argument("zkSoftmax::prove_fs: X and Y must have same size");

    auto u_Y = transcript.challenge_vec(label_prefix + "/softmax/u_Y", ceilLog2(N));
    auto v_check = transcript.challenge_vec(label_prefix + "/softmax/v_check", ceilLog2(N));
    auto r_seg = transcript.challenge_fr(label_prefix + "/softmax/r_seg");
    auto alpha_seg = transcript.challenge_fr(label_prefix + "/softmax/alpha_seg");
    auto beta_seg = transcript.challenge_fr(label_prefix + "/softmax/beta_seg");

    // 1) Bind Y = hadamard product of Y_segments
    auto claim_Y = Y(u_Y);
    multi_hadamard_sumchecks_prove_fs(claim_Y, Y_segments, u_Y, transcript, proof_messages, label_prefix + "/softmax");

    // 2) Each segment lookup proof
    for (uint i = 0; i < L; ++i) {
        least_significant_segments[i].prove_fs(X_segments[i], m_segments[i], alpha_seg, beta_seg, u_Y, transcript, proof_messages, label_prefix + "/softmax/ls/" + std::to_string(i));
    }
    for (uint i = L; i < K; ++i) {
        other_segments[i - L].prove_fs(X_segments[i], Y_segments[i - L], m_segments[i], r_seg, alpha_seg, beta_seg, u_Y, transcript, proof_messages, label_prefix + "/softmax/os/" + std::to_string(i));
    }

    // 3) Reconstruction check at a separate random point
    vector<Fr_t> X_seg_claims;
    for (uint i = 0; i < K; ++i) X_seg_claims.push_back(X_segments[i](v_check));
    Fr_t minus_X_shifted_claim = {0, 0, 0, 0, 0, 0, 0, 0};
    for (int i = bs.size() - 1; i >= 0; --i) {
        minus_X_shifted_claim = minus_X_shifted_claim * Fr_t({bs[i], 0, 0, 0, 0, 0, 0, 0}) + X_seg_claims[i];
    }
    vector<Fr_t> v_Y_(v_check.begin() + ceilLog2(n), v_check.end());
    auto shift_open = shift(v_Y_);
    auto X_open = X(v_check);
    if (shift_open - minus_X_shifted_claim != X_open) throw std::runtime_error("zkSoftmax::prove_fs: reconstruction check failed");
    return X_open;
}

Fr_t zkSoftmax::verify_fs(
    const FrTensor& Y, const FrTensor& X, const FrTensor& shift, const FrTensor& X_shifted,
    const vector<FrTensor>& X_segments, const vector<FrTensor>& Y_segments, const vector<FrTensor>& m_segments,
    Transcript& transcript, const std::vector<uint8_t>& proof_messages, size_t& proof_off, const std::string& label_prefix)
{
    uint N = Y.size;
    if (X.size != N) throw std::invalid_argument("zkSoftmax::verify_fs: X and Y must have same size");
    ...
    auto claim_Y = Y(u_Y);
    multi_hadamard_sumchecks_verify_fs(claim_Y, Y_segments, u_Y, transcript, proof_messages, proof_off, label_prefix + "/softmax");

    for (uint i = 0; i < L; ++i) {
        least_significant_segments[i].verify_fs(X_segments[i], m_segments[i], alpha_seg, beta_seg, u_Y, transcript, proof_messages, proof_off, label_prefix + "/softmax/ls/" + std::to_string(i));
    }
    for (uint i = L; i < K; ++i) {
        other_segments[i - L].verify_fs(X_segments[i], Y_segments[i - L], m_segments[i], r_seg, alpha_seg, beta_seg, u_Y, transcript, proof_messages, proof_off, label_prefix + "/softmax/os/" + std::to_string(i));
    }
    ...
    auto shift_open = shift(v_Y_);
    auto X_open = X(v_check);
    if (shift_open - minus_X_shifted_claim != X_open) throw std::runtime_error("zkSoftmax::verify_fs: reconstruction check failed");
    return X_open;
}
